...
public class MarketingCampaignsServiceImpl implements MarketingCampaignsService {
    ...
    public ResponseData getWorkshopsByCategories() {
        try {
            ...
            List<CategoryWorkshopDTO> categoryWorkshops = categories.stream()
                    .map(category -> {
                        try {
                            ...
                            for (MarketingCampaigns campaign : campaigns) {
                                ...
                                if (wmcs != null) {
                                    logger.info("Campaign {} has {} workshop marketing campaigns",
                                            campaign.getCampaignId(), wmcs.size());
                                    for (WorkshopMarketingCampaign wmc : wmcs) {
                                        if (wmc.getWorkshop() != null) {
                                            logger.info("Workshop: {} ({})",
                                                    wmc.getWorkshop().getWorkshopId(),
                                                    wmc.getWorkshop().getWorkshopTitle());
                                        }
                                    }
                                }
                            }
                                    ...
                                    for (WorkshopMarketingCampaign wmc : campaign.getWorkshopMarketingCampaigns()) {
                                        if (wmc.getWorkshop() != null) {
                                            ...
                                            if (workshop.getInstructor() != null) {
                                                response.setCreateBy(workshop.getInstructor().getFirstName() + " " +
                                                        workshop.getInstructor().getLastName());
                                            }
                            ...
                            logger.info("Category {} has {} workshops in final response",
                                    category.getCategoryId(), workshops.size());
